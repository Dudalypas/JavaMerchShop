package kursinis.viljamas.hibernate;
...
public class ShopHibernate extends GenericHibernate {
    ...
    public List<Cart> filterOrders(Customer customer, Manager manager , LocalDate from, LocalDate to) {
        ...
        try {
            ...
            query.select(root).where(
                    cb.and(
                            ...
                            cb.between(root.get("dateCreated"), from, to)
                    )
            );
    ...
    public Manager getManagerByName(String name) {
        ...
        try {
            ...
            CriteriaQuery<Manager> query = cb.createQuery(Manager.class);
            Root<Manager> root = query.from(Manager.class);
            query.select(root).where(cb.equal(root.get("name"), name)); // Assuming the name property in Manager entity
            Query q = em.createQuery(query);
            return (Manager) q.getSingleResult();
        } catch (NoResultException e) {
            return null;
    ...
    public <T> T getUserByLogin(String login) {
        EntityManager em = getEntityManager();
        try {
            Manager manager = getManagerByLogin(login);
            return manager == null ? (T) getCustomerByLogin(login) : (T) manager;
        } catch (NoResultException e) {
            return null;
        } finally {
            if (em != null) em.close();
